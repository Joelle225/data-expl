...
for fold, (train_group_indices, val_group_indices) in enumerate(kf.split(group_keys)):
    ...
    train_dataset_sklearn = SlidingWindowPoseDataset(
    ...
    print(f"Training dataset size (sklearn): {len(train_dataset_sklearn)}")
    ...
    X_train_list = [sample[0] for sample in train_dataset_sklearn]
    y_train_list = [sample[1] for sample in train_dataset_sklearn]
    ...
    print(f"Num pos for RF training: {num_pos_train}, Num neg for RF training: {num_neg_train}")
    ...
    if num_pos_train == 0 or num_neg_train == 0:
        ...
        # Adjust class_weight if only one class, or skip. For now, RF might handle it or error.
        # Using class_weight='balanced' is generally robust.
        if rf_class_weight is None: # If not using 'balanced'
            print("Consider setting class_weight='balanced' in RandomForestClassifier.")
            # continue # Optionally skip fold if strict two-class requirement
    ...
    if len(val_dataset_sklearn) > 0:
        ...
        print(f"Skipping validation metrics for fold {fold+1} as validation set is empty.")
...
if not overall_y_labels_accumulated:
    ...
    y_scores_overall = np.concatenate(overall_y_scores_accumulated) # These are probabilities
    ...
    try:
        ...
        # For P/R/F1, convert scores to binary predictions
        y_preds_overall = (y_scores_overall > 0.5).astype(int)
        ...
        num_pos = (y_preds_overall == 1).sum()
        num_neg = (y_preds_overall == 0).sum()
        percent_pos = 100.0 * num_pos / num_total
        percent_neg = 100.0 * num_neg / num_total

        print("\n=== Overall Ensemble Performance (Random Forest) ===")
